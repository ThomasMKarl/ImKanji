...
auto hdataDB = readHdata();
auto flashcardDB_gr = readFlashcards();
...
void Grapheme_Window::draw(const char* title, bool* p_open)
{
        ...
        for (const auto & flashcard : flashcardDB_gr)
        {
            ...
            for (const auto & sel : selected)
            {
                if (sel.empty())
                    continue;
                allEmpty = false;
                bool found = std::find_if(std::begin(radicals), std::end(radicals), [sel](const auto & in){ return sel == graphemeList.at(in - 1); }) != std::end(radicals);
                if (!found)
                {
                    match = false;
                    break;

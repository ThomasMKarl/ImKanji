...
auto flashcardDB_c = imkanji::readFlashcards();
auto hdataDB_c = imkanji::readHdata();

struct Helper
{
  imkanji::Index fIndex{};
  imkanji::Index hIndex{};
  imkanji::String kanji{};
  imkanji::String hString{};
};

void imkanji::window::exportHtml(const char * path, const std::pair<imkanji::Index, imkanji::Index> & fromTo)
{
  if (path == nullptr)
    return;
       ...
       ++it)
  {
    ...
    strokeMap.at(strokes - 1).emplace_back(Helper{.fIndex = flashcard.fIndex, .hIndex = flashcard.hIndex, .kanji = flashcard.kanji, .hString = h});
  }
...
void imkanji::window::exportLatex(const char * path, const std::pair<imkanji::Index, imkanji::Index> & fromTo)
{
  if (path == nullptr)
    return;
...
void imkanji::window::exportTable(const char * path, const std::pair<imkanji::Index, imkanji::Index> & fromTo)
{
  if (path == nullptr)
    return;
...
imkanji::Index toIndex(imkanji::String & val)
{
  uint8_t counter = 0;
  ...
  return std::min(static_cast<imkanji::Index>(540), static_cast<imkanji::Index>(std::stoul(val)));
}

bool check(const std::pair<imkanji::Index, imkanji::Index> & range)
{
  if (range.first == 0)
    return false;
  if (range.second == 0)
...
void imkanji::window::Config::draw(const char * title, bool * p_open)
{
  ...
  if (ImGui::Button("export"))
  {
    ...
    auto path = FileDialog::saveFile(filter,
                                     nullptr,
                                     (String{"kanji_sorted_"} + std::to_string(fromTo.first) + "-" + std::to_string(fromTo.second)).c_str());
    if (!path.first.empty())
  ...
  for (uint8_t index = 1; index <= numberOfCardboxLevels; ++index)
  {
    if (ImGui::Button(std::to_string(index).c_str()))
    {
      ...
      flashcardWindow = Flashcard{index, hide, kanjiFuri, shuffle};
  ...
  if (ImGui::Button("train kanji") && check(fromTo))
  {
    ...
    if (flashcardWindow.has_value() && tableWindow != nullptr)
      tableWindow->unsubscribe(&(flashcardWindow.value()));
    flashcardWindow = Flashcard{fromTo, hide, kanjiFuri, shuffle};
    if (tableWindow != nullptr)
      tableWindow->subscribe(&(flashcardWindow.value()));
  }

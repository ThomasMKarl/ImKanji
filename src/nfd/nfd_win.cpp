...
{
  T * data;
  Release_Guard(T * releasable) noexcept :
      data(releasable) {}
...
{
  T * data;
  Free_Guard(T * freeable) noexcept :
      data(freeable) {}
...
{
  T * data;
  FreeCheck_Guard(T * freeable = nullptr) noexcept :
      data(freeable) {}
  ~FreeCheck_Guard()
  {
    if (data)
                            ...
                            const nfdnchar_t * defaultName)
{
  ...
  if (SUCCEEDED(result))
  {
    ...
    *outPath = filePath;
    ...
    UINT _fileTypeIndex;
    result = fileSaveDialog->GetFileTypeIndex(&_fileTypeIndex);
    if (!SUCCEEDED(result))
    {
      NFDi_SetError("Could not get file type index from dialog.");
      return NFD_ERROR;
    }
    *fileTypeIndex = static_cast<unsigned char>(_fileTypeIndex - 1);
                             ...
                             const nfdu8char_t * defaultName)
{
  ...
  nfdresult_t res = NFD_SaveDialogN(
      &outPathN, fileTypeIndex, filterItemsNGuard.data, count, defaultPathNGuard.data, defaultNameNGuard.data);
